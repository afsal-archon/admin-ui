...
const AgentConsole = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      console.log("📌 Loaded chat list from localStorage");
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "snapshot") {
        ...
        setChats(normalized);
        localStorage.setItem("chat_list", JSON.stringify(normalized));
      }
  ...
  const handleConversationClosed = (id) => {
    const updated = chats.filter((c) => c.conversation_id !== id);
